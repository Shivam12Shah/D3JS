...
const CandlestickChart = ({ data }) => {
  ...
  useEffect(() => {
    ...
    // Create base scales
    baseXScaleRef.current = d3.scaleTime()
      .domain(d3.extent(data, d => d.date))
      .range([margin.left, width - margin.right]);

    baseYScaleRef.current = d3.scaleLinear()
    ...
    const candleWidth = (width - margin.left - margin.right) / data.length * 1;

    // Draw candlesticks
    const candles = svg.append('g')
      .attr('class', 'candles');

    candles.selectAll('g.candle')
      .data(data)
      .enter()
      .append('g')
      .attr('class', 'candle')
    ...
    const zoom = d3.zoom()
      ...
      .on('zoom', (event) => {
        ...
        currentXScale.domain(transform.rescaleX(baseXScaleRef.current).domain());
        currentYScale.domain(transform.rescaleY(baseYScaleRef.current).domain());
        updateChart();
      })
